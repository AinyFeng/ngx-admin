...
import {ColdchainSelectedNodeService, SensorDeviceService} from '@tod/svs-common-lib';
import {AddEditSencorComponent} from '../dialoge/add-edit-sencor/add-edit-sencor.component';
...
export class SensorComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private deviceSvc: SensorDeviceService
  ) {
  ...
  add() {
    ...
    const modal = this.modalService.create({
      nzTitle: '新增传感器',
      nzContent: AddEditSencorComponent,
      nzWidth: 870,
      nzClosable: false,
      nzMask: false,
      nzComponentParams: {
  ...
  delete() {
    ...
    this.modalService.confirm({
      ...
      nzOnOk: () =>
        // 调删除的逻辑
        this.deviceSvc.deleteSensorDevice(ids, resp => {
          // 解析表格数据
          if (resp && resp.code === 0) {
            this.searchData();
            console.log('删除成功！');
  ...
  searchData(page = 1) {
    ...
    this.refreshStatus();
    const params = {
      sensorName: this.queryForm.value.deviceName,
    ...
    this.loading = true;
    this.deviceSvc.querySensorDeviceInfo(params, resp => {
