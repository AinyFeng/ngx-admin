...
import {
  DateUtils, DicDataService,
  VaccManufactureDataService,
  VacStockStorageApi,
  VacStockApprovalApiService, PovDataInit
...
@Component({
  selector: 'uea-stock-write-off',
  templateUrl: './stock-write-off.component.html',
  styleUrls: ['./stock-write-off.component.scss']
})
export class StockWriteOffComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private povInfoSvc: PovDataInit
  ) {
    // 获取上产厂商
  ...
  ngOnInit() {
    this.povInfo = this.povInfoSvc.getPovData();
    this.orderForm = this.fb.group({
      ...
      orderType: [this.updateOrderDate ? this.updateOrderDate.orderType : '2', [Validators.required]], // 类型 2--报废
      ...
      supplyorgName: null, // 供货单位，当前登录用户所在单位
      supplyorgCode: null, // 供货单位编码，当前登录用户所在单位编码
      consignorName: null, // 供货单位经手人，当前登录用户编码
    });
  }
  ...
  queryDetail() {
    ...
    this.vacStockApprovalSvc.queryStockApprovalDetail(params, resp => {
      ...
      this.vaccData.forEach(item => {
        item.checked = true;
        item['breakNum'] = item.vaccNum;
      });
    });
  }
  ...
  submitForm(): void {
    ...
    /* console.log('需要报废的疫苗信息-----', this.vaccData);*/
    // 检查疫苗报废数量是否有效和是否选择
    ...
    if (this.updateOrderDate) {
      ...
      this.vaccData.forEach(item => item.vaccNum = item.breakNum);
      ...
      this.vasStockStorageApiSvc.scrap(params, res => {
        console.log('疫苗报废接口返回值', res);
        if (res.code === 0) {
          this.notifierService.notify('success', '成功');
          this.resetTable();
          this.calculateTotal(); // 清空数量
  ...
  checkStorageNumSelectedAndValid() {
    ...
    // 检查数量有效性
    const dv = d.filter(b => !b.hasOwnProperty('breakNum') || !b.breakNum || b.breakNum === 0);
    if (dv.length > 0) {
      this.modalService.warning({
        nzTitle: '消息提示',
        nzContent: `<p>报废数量不合法，请重新填写</p>`,
        nzMaskClosable: true
      });
      return false;
    }
  ...
  calculateTotal() {
    this.total = 0;
    this.vaccData.forEach(d => this.total += d['breakNum']);
  }
