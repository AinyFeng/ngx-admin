...
export class StockOutComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    const sub = this.route.queryParams.subscribe(resp => {
      if (resp.title) {
        this.updateOrderDate = JSON.parse(resp.info);
  ...
  ngOnInit() {
    ...
    if (officeData) {
      ...
      this.orderForm.get('supplyorgCode').setValue(officeData.organizationCode);
    }
    // 获取组织树数据
  ...
  selectDistrict() {
    ...
    modal.afterClose.pipe(take(1)).subscribe(res => {
      if (res) {
        console.log(res);
        const organizationGrade = res['organizationGrade'].trim();
        const officeOrganizationGrade = this.officeInfo['organizationGrade'].trim();
        const selectedAreaCode = res['areaCode'];
        if (organizationGrade < officeOrganizationGrade) {
          this.modalService.warning({
            nzTitle: '提示',
            nzContent: '不可选择此单位，请选择本单位的下级单位',
            nzMaskClosable: true
          });
          return;
        }
        if (officeOrganizationGrade - organizationGrade > 10) {
          this.modalService.warning({
            nzTitle: '提示',
            nzContent: '不可跨级下发，请选择本单位的下级单位',
            nzMaskClosable: true
          });
          return;
        }
        // 说明是省疾控，只能下发给市疾控的直属单位
        if (officeOrganizationGrade === '10') {
          const areaCode = this.officeInfo['areaCode'].substr(0, 2);
          if (selectedAreaCode.substr(0, 2) !== areaCode) {
            this.modalService.warning({
              nzTitle: '提示',
              nzContent: '不可跨区域下发，请选择本单位的下级单位',
              nzMaskClosable: true
            });
            return;
          }
        }
        // 说明是市疾控，只能下发给市疾控的直属单位
        if (officeOrganizationGrade === '20') {
          const areaCode = this.officeInfo['areaCode'].substr(0, 4);
          if (selectedAreaCode.substr(0, 4) !== areaCode) {
            this.modalService.warning({
              nzTitle: '提示',
              nzContent: '不可跨区域下发，请选择本单位的下级单位',
              nzMaskClosable: true
        ...
        // 说明是区县疾控，只能下发给区县疾控下的门诊部门
        if (officeOrganizationGrade === '30') {
          const areaCode = this.officeInfo['areaCode'];
  ...
  checkStorageNumSelectedAndValid() {
    ...
    const smallPackageRet = this.stockData.filter(sd => {
      if (sd.hasOwnProperty('smallPackage') && sd['smallPackage']) {
        const smp = parseInt(sd['smallPackage'], 10);
        const vacNum = parseInt(sd['vaccNum'], 10);
        if (vacNum % smp !== 0) {
          return sd;
        }
      }
    });
    console.log(smallPackageRet);
