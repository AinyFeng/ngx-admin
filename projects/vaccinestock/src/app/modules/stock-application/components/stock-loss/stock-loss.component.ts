...
import {
  ...
  VaccManufactureDataService, VacStockStorageApi
...
export class StockLossComponent implements OnInit {
  ...
  constructor(
    ...
    this.orderForm = fb.group({
      ...
      orderType: [this.updateOrderDate ? this.updateOrderDate.orderType : '6', [Validators.required]], // 类型 报损
  ...
  submitForm(): void {
    ...
    if (this.updateOrderDate) {
      ...
      this.vaccData.forEach(item => item.vaccNum = item.lossNum);
  ...
  checkStorageNumSelectedAndValid() {
    ...
    // 检查数量有效性
    const dv = d.filter(b => !b.hasOwnProperty('lossNum') || !b.lossNum || b.lossNum === 0);
    if (dv.length > 0) {
      this.modalService.warning({
        nzTitle: '消息提示',
  ...
  calculateTotal() {
    this.total = 0;
    this.vaccData.forEach(d => this.total += d['lossNum']);
  }
