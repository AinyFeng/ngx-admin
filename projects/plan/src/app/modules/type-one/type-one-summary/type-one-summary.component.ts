...
@Component({
  selector: 'uea-type-one-summary',
  templateUrl: './type-one-summary.component.html',
  styleUrls: ['./type-one-summary.component.scss']
})
export class TypeOneSummaryComponent implements OnInit {
  ...
  query(): void {
    ...
    this.planConfigService.statisticalPlanMonth(param, resp => {
      if (resp['code'] === 0) {
  ...
  queryTabSet(storeCode?: string): void {
    ...
    this.planConfigService.statisticalBranchPlanMonth(param, resp => {
      if (resp['code'] === 0) {
        const data = resp['data'];
        this.tabList = data;
      }
  ...
  exportFile() {
    ...
    this.planConfigService.statisticalExportFilePlanMonth(param, (blob, fileName, type) => {
