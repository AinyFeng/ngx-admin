...
@Component({
  selector: 'uea-type-one-yearly-audit',
  templateUrl: './type-one-yearly-audit.component.html',
  styleUrls: ['./type-one-yearly-audit.component.scss']
})
export class TypeOneYearlyAuditComponent implements OnInit {
  ...
  query(reset = false, event?: number): void {
    ...
    const param = {
      ...
      planDate: this.queryForm.controls['planDate'].value ? DateUtils.getFormatTime(new Date(this.queryForm.controls['planDate'].value), 'YYYY') : null,
      pageEntity: {
        page: this.page,
