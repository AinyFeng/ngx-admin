...
@Component({
  selector: 'uea-type-one-monthly-details',
  templateUrl: './type-one-monthly-details.component.html',
  styleUrls: ['./type-one-monthly-details.component.scss']
})
export class TypeOneMonthlyDetailsComponent implements OnInit {
  @Input()
  tabIndex: number;
  ...
  @ViewChild('packNumInput', { static: false, read: ElementRef }) packNumInput: ElementRef;
  ...
  handleClick(e: MouseEvent): void {
    ...
    if (this.editPackNum && this.packNumInput && this.packNumInput.nativeElement !== e.target) {
      this.editPackNum = null;
    }
  ...
  checkPlanMonth() {
    ...
    this.planConfigService.checkPlanMonth(param, resp => {
      console.log(resp);
      if (resp['code'] === 0) {
        if (resp['data']) {
  ...
  getPlanMonthDetailWithYearPlanDetails(event?: any) {
    ...
    this.planConfigService.getPlanMonthDetailWithYearPlanDetails(param, resp => {
      if (resp['code'] === 0) {
  ...
  getPlanMonth() {
    this.planConfigService.getPlanMonth({ planMonthCode: this.planMonthCode }, resp => {
      if (resp['code'] === 0) {
  ...
  getMonthDetail() {
    ...
    this.planConfigService.getMonthDetail(param, resp => {
      if (resp['code'] === 0) {
        this.dataSet = resp['data'];
      }
    });
  ...
  exportPlan() {
    ...
    this.planConfigService.exportFilePlanMonth(param, (blob, fileName, type) => {
  ...
  savePlan(isCheck = false, affirmStatus?: string) {
    ...
    const planMonth = {
      ...
      type: '1',
      delFlag: '0',
      createBy: !!this.planMonth['createBy'] ? null : this.queryForm.controls['planBy'].value,
    ...
    const param = {
      platformPlanMonth: planMonth,
      platformPlanMonthDetailList: this.dataSet
    };
    this.planConfigService.savePlanMonth(param, resp => {
      if (resp.code === 0) {
        if (resp['data']) {
