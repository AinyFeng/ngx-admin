...
@Component({
  selector: 'uea-type-two-monthly-list',
  templateUrl: './type-two-monthly-list.component.html',
  styleUrls: ['./type-two-monthly-list.component.scss']
})
export class TypeTwoMonthlyListComponent implements OnInit {
  ...
  query(reset = false, event?: number): void {
    ...
    this.planConfigService.queryPlanMonthToll(param, resp => {
  ...
  delete(planMonthCode) {
    this.planConfigService.deletePlanMonthToll({ planMonthCode: planMonthCode }, resp => {
      console.log(resp);
