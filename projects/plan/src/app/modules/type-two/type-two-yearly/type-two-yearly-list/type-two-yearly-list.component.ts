...
@Component({
  selector: 'uea-type-two-yearly-list',
  templateUrl: './type-two-yearly-list.component.html',
  styleUrls: ['./type-two-yearly-list.component.scss']
})
export class TypeTwoYearlyListComponent implements OnInit {
  ...
  query(reset = false, event?: number): void {
    ...
    const param = {
      ...
      quarter: this.queryForm.controls['quarter'].value === 'null' ? null : this.queryForm.controls['quarter'].value,
  ...
  delete(planYearCode) {
    this.planConfigService.deletePlanYearToll({ planYearCode: planYearCode }, resp => {
      console.log(resp);
  ...
  monthlyFillIn(planYearCode) {
    this.router.navigate(['/modules/plan/typeTwo/monthly/list'], { queryParams: { planYearCode: planYearCode } });
  }
